...
export async function initDuckDB() {
  ...
  globalCache.initPromise = (async () => {
    ...
    // Register only the local parquet file stored in public/data.
    const parquetAlias = 'data_local.parquet';
    await _db.registerFileURL(parquetAlias, PARQUET_URL, duckdb.DuckDBDataProtocol.HTTP, false);
    const sampleResult = await globalCache.conn.query(`SELECT * FROM '${parquetAlias}' LIMIT 1`);
    globalCache.parquetSource = PARQUET_URL;
    ...
    const query = `SELECT ${selectClause} FROM '${parquetAlias}'`;
