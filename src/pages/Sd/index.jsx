...
export const Sd = ({ year, restartKey = 0, onRestart = () => {} }) => {
  const [iframeLoading, setIframeLoading] = useState(true);
  const embedUrl = getUrlForTypeAndYear('SD', year);
    ...
    return (
      <div className="flex items-center justify-center h-screen bg-gray-50 text-gray-800">
        <div className="p-8 bg-white rounded-xl shadow-sm border border-gray-200 text-center">
          ...
          <p>Konfigurasi URL SD untuk tahun {year} belum tersedia di file .env.</p>
        </div>
      </div>
    );
  }
      ...
      >
        {iframeLoading && (
          <DashboardLoadingOverlay
            title={`Memuat Dashboard SD (${year})`}
            message={`Sedang menyiapkan tampilan dashboard SD tahun ${year}...`}
            fullScreen={false}
          />
        )}
        {/* Konten */}
